...
- **Menus bloqueados (não escondidos):** `auth.js` exibe os itens de menu sem permissão como **desabilitados** (`data-perm-blocked=true`) — ficam acinzentados, com cadeado `🔒` ao lado do nome, `cursor: not-allowed`, tooltip "Acesso restrito — em implementação para o perfil X" e um toast inferior ao clicar. CSS em `nav-profile.css`. A função `marcarBloqueado()` é compartilhada pelo cache imediato (anti-flash) e `aplicarPermissoesNav()`. O backend continua aplicando o gate real (`requireModulo`).
