...
- **Fontes de nota externas**: tabela `classroom_grupo_fontes` (grupo_id, fonte_grupo_id, peso) — permite importar notas de grupos de outras disciplinas; endpoints `PUT /groups/:id/fontes` e `GET /all-groups`; frontend com modal de configuração no grupo (selecionar grupo fonte + peso %); resumo do grupo inclui notas externas ponderadas; badge "📥 N fontes" no card do grupo; matching de alunos entre disciplinas feito por email; backend retorna `fontesAtividades` per-student com status de cada atividade da fonte; frontend exibe barras coloridas das atividades da fonte após separador "│" na linha do aluno; detalhe do aluno (click) mostra seção dedicada por fonte com header (📥 nome, peso, pts) e linhas por atividade; tabs de filtro (Todas/Realizadas/Entrou/Não realizadas) incluem contagem combinada (grupo + fontes); seções de fonte são ocultadas automaticamente quando filtro não mostra nenhuma atividade da fonte
