...
- **Fontes de nota externas**: tabela `classroom_grupo_fontes` (grupo_id, fonte_grupo_id, peso) — permite importar notas de grupos de outras disciplinas; endpoints `PUT /groups/:id/fontes` e `GET /all-groups`; frontend com modal de configuração no grupo (selecionar grupo fonte + peso %); resumo do grupo inclui notas externas ponderadas; badge "📥 N fontes" no card do grupo; matching de alunos entre disciplinas feito por email
